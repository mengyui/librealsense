...
public:
    ...
    void DisableStream(int stream) {
        cfg.disable_stream(static_cast<rs2_stream>(stream));
    }
    ...
    rs2::device QueryDevice(void) {
        return pipe.get_active_profile().get_device();
    }
...
public:
    ...
    void OnChipCalibration(void) {
        if (0 == devices_info.size()) return;
        std::stringstream ss;
        ss << "OnChipCalibration BEGIN" << std::endl;
        if (!devices_info[0]) devices_info[0] = new DevicePipe(*ctx, 0);

        devices_info[0]->DisableStream(RS2_STREAM_INFRARED);
        devices_info[0]->DisableStream(RS2_STREAM_COLOR);
        devices_info[0]->EnableStream(RS2_STREAM_DEPTH, 256, 144, 90, RS2_FORMAT_Z16, nullptr);
        devices_info[0]->Start();
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
        try {
            auto calib_dev = devices_info[0]->QueryDevice().as<rs2::auto_calibrated_device>();
            ...
            // SHOULD BE MODIFIED, IF NOT DEFAULT OCC.
            // REF: on_chip_calib_manager::calibrate
            json_ss << "{\n \"calib type\":" << 0 <<
               ",\n \"speed\":" << speed <<
            ...
            auto json = json_ss.str();
            float health = 0;
            ss << "run_on_chip_calibration START" << std::endl;
            auto new_table = calib_dev.run_on_chip_calibration(json, &health, [&](const float progress) {
                ss << "" << progress << std::endl;
            });
            ss << "health: " << health << std::endl;
            ss << "new_table: " << std::endl;
            ss << std::hex; for (auto c: new_table) {
                ss << (int)c << " ";
            } ss << std::dec; ss << std::endl;

            // APPLY NEW CALIBRATION TABLE.
            calib_dev.set_calibration_table(new_table);

            if (health < 0.25f) {
                //// CHECK THE DEPTH QUALITY, THEN WRITE TO THE DEVICE.
                calib_dev.write_calibration();
            }
            ss << "run_on_chip_calibration STOP" << std::endl;
        } catch (...) {
            ss << "OnChipCalibration EXCEPTION" << std::endl << std::endl << std::endl << std::endl << std::endl;
        }
        devices_info[0]->Stop();

        ss << "OnChipCalibration END" << std::endl;
        this->profileList = ss.str();
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_example_realsense_1app_MainActivity_nOnChipCalibration(JNIEnv *env, jclass type) {
    RealSenseWrapper::GetInstance()->OnChipCalibration();
}
